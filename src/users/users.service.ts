...
export class UsersService {
   ...
   * @param {UpdateUserInput} fieldsToUpdate The user can update their email, email, password, or enabled. If
  ...
  async update(
    ...
    if (Object.entries(fieldsToUpdate).length > 0) {
      user = await this.userModel.findOneAndUpdate({ email: email }, fields, {
        new: true,
        runValidators: true,
      });
    } else {
   ...
   * @returns {(Promise<UserDocument | undefined>)} Returns undefined if the code or the email is wrong
   * @memberof UsersService
   */
  async resetPassword(
    email: string,
   ...
   * Creates a user
   *
   * @param {CreateUserInput} createUserInput email and password. Email must be
